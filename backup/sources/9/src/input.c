...
// After updating the position variable,first get the next character:	
	Entity *empty = NULL;
	CheckNextChar('@',PLAYER,y,x,level,empty); 
...
/////////////// MONSTER MOVEMENT! ///////////////////////
	for(int i=0; i<level.mons; i++)
	{
		...
		{
			yx = PathFindFollow(yx,y,x,level.player->Position.yPos,level.player->Position.xPos,'.','#',':');
		} else if(level.monsters[i]->mode==RANDOM_S)
		{
			yx = PathFindRandom(yx,y,x,'.',RANDOM_S);
		} else if(level.monsters[i]->mode==RANDOM_X)
		{
			yx = PathFindRandom(yx,y,x,'.',RANDOM_X);
		} else
		{
			yx = PathFindRandom(yx,y,x,'.',RANDOM_X);
		}
		...
		level.monsters[i] = CheckNextChar(level.monsters[i]->symbol,MONSTER,y,x,level,level.monsters[i]);
	}
//////////////////////////////////////////////////////////
