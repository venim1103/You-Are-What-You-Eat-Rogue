...
//		level->lvl = r;
	...
	// HANDLE INPUT:
		HandleInput(level[level->lvl].player, level[level->lvl].map, level[level->lvl].rooms);
	//////////// FOR MOVEMENT BUG ////////////////
		co = level[level->lvl].map[level[level->lvl].player->Position.yPos][level[level->lvl].player->Position.xPos];
		if(co!='@') // IF NOT STAY STILL SO THAT PREV CHAR is @:
		{ // Update the last character  
			level[level->lvl].player->Position.ch = co;
		}
	/////////////////////////////////////////////
		if(c=='L')
		{
