	...
	{  
		...
		switch(c)
		{
			...
			case '#':
				...
				{
					monster = MonsterMove(monster,":",monster->symbol,nexty,nextx);
				} else if(monster->Position.type==UNKNOWN) // IF STOPPED
				{
					...
					{
						sprintf(buf,"%c",level.map_backup[y][x]);	
					}
					monster = MonsterMove(monster,buf,monster->symbol,nexty,nextx);
				} else
				{ 
